...
fn bench_matched(c : &mut Criterion) {
    ...
    keys1.extend(vec!['H';1].iter());
    keys1.extend(vec!['K';1].iter());
    keys1.extend(vec!['D';1].iter());
    keys1.extend(vec!['O';1].iter());
    keys1.extend(vec!['R';1].iter());

    keys2.extend(vec!['H';1].iter());
    keys2.extend(vec!['K';1].iter());
    keys2.extend(vec!['D';1].iter());
    keys2.extend(vec!['O';1].iter());
    keys2.extend(vec!['R';1].iter());
    ...
    c.bench_function(
        "Compare 2 Strings",
        |b| b.iter(|| matched(element.0.clone(),element.1.clone()))
    );
}
...
fn inuse_bench_matched(c : &mut Criterion) {
    ...
    c.bench_function(
        ...
        |b| b.iter(|| in_matched(element.0.clone(),element.1.clone()))
    );
}
